...
// ════════════════════════════════════════════════════════════
// C.7 — Branding Controller API
// ════════════════════════════════════════════════════════════
console.log('\n── C.7 Branding Controller API ─────────────────────────────────');
{
  const b=makeBrowser({scriptDataset:{business:'biz_br'}});
  await runBundle(b); const sdk=b.sdk;

  check('C7 BR1: branding exposed on sdk',             sdk.branding!==null,                       typeof sdk.branding);
  check('C7 BR1: runtime.branding===sdk.branding',     sdk.runtime.branding===sdk.branding,       '');
  check('C7 BR1: applyBranding() is a function',       typeof sdk.applyBranding==='function',     '');
  check('C7 BR1: resetBranding() is a function',       typeof sdk.resetBranding==='function',     '');
  check('C7 BR1: getActiveBrand() is a function',      typeof sdk.getActiveBrand==='function',    '');
}
...
console.log('\n── C.7 Color Engine ────────────────────────────────────────────');
{
  const b=makeBrowser({scriptDataset:{business:'biz_ce'}});
  await runBundle(b); const sdk=b.sdk;

  sdk.applyBranding({colors:{primary:'#ff0000'}});
  const brand=sdk.getActiveBrand();

  check('C7 CE1: getActiveBrand() returns object',     typeof brand==='object'&&brand!==null,     '');
  check('C7 CE2: palette.primary applied',             brand?.palette?.primary==='#ff0000',       `primary=${brand?.palette?.primary}`);
  check('C7 CE3: primaryHover derived (darker)',       brand?.palette?.primaryHover!==undefined,  `hover=${brand?.palette?.primaryHover}`);
  check('C7 CE4: primaryPressed derived',              brand?.palette?.primaryPressed!==undefined,'');
  check('C7 CE5: primaryForeground is #000 or #fff',   ['#000000','#ffffff'].includes(brand?.palette?.primaryForeground??''), `fg=${brand?.palette?.primaryForeground}`);
  check('C7 CE6: secondary has default',               typeof brand?.palette?.secondary==='string','');
  check('C7 CE7: palette has danger field',            typeof brand?.palette?.danger==='string',  '');

  // Reset then apply dark defaults
  sdk.resetBranding();
  sdk.applyBranding({themeMode:'dark'});
  const darkBrand=sdk.getActiveBrand();
  check('C7 CE8: dark mode sets palette',              darkBrand?.palette?.primary!==undefined,   `primary=${darkBrand?.palette?.primary}`);
}

// ════════════════════════════════════════════════════════════
// C.7 — Typography Engine
// ════════════════════════════════════════════════════════════
console.log('\n── C.7 Typography Engine ───────────────────────────────────────');
{
  const b=makeBrowser({scriptDataset:{business:'biz_ty'}});
  await runBundle(b); const sdk=b.sdk;

  sdk.applyBranding({typography:{fontFamily:'Georgia, serif',headingScale:'large',bodyScale:'large',weight:'bold'}});
  const brand=sdk.getActiveBrand();

  check('C7 TY1: fontFamily applied',                  brand?.typography?.fontFamily==='Georgia, serif', `ff=${brand?.typography?.fontFamily}`);
  check('C7 TY2: headingSize for large scale',         brand?.typography?.headingSize==='24px',   `h=${brand?.typography?.headingSize}`);
  check('C7 TY3: bodySize for large scale',            brand?.typography?.bodySize==='16px',      `b=${brand?.typography?.bodySize}`);
  check('C7 TY4: fontWeight bold=700',                 brand?.typography?.fontWeight==='700',     `w=${brand?.typography?.fontWeight}`);
  check('C7 TY5: lineHeight has default',              typeof brand?.typography?.lineHeight==='string','');
  check('C7 TY6: letterSpacing has default',           typeof brand?.typography?.letterSpacing==='string','');
}
...
console.log('\n── C.7 Assets Manager ──────────────────────────────────────────');
{
  const b=makeBrowser({scriptDataset:{business:'biz_am'}});
  await runBundle(b); const sdk=b.sdk;
  ...
  sdk.applyBranding({
    assets:{
      ...
      launcherIcon:'<svg><circle/></svg>',
    }
  });
  const brand=sdk.getActiveBrand();
  const d=sdk.getDiagnostics();

  check('C7 AS1: logo configured',                     d.logoConfigured===true,                   `logo=${d.logoConfigured}`);
  check('C7 AS2: avatar configured',                   d.avatarConfigured===true,                 `av=${d.avatarConfigured}`);
  check('C7 AS3: assets.logo in brand',                brand?.assets?.logo==='https://example.com/logo.png', '');
  check('C7 AS4: assets.launcherIcon (SVG) accepted',  typeof brand?.assets?.launcherIcon==='string','');

  // Invalid asset should not crash
  let threw=false;
  try { sdk.applyBranding({assets:{logo:'not-a-url'}}); }
  catch { threw=true; }
  check('C7 AS5: invalid asset does not throw',        !threw,                                    'no throw');
}
...
console.log('\n── C.7 White Label ─────────────────────────────────────────────');
{
  const b=makeBrowser({scriptDataset:{business:'biz_wl'}});
  await runBundle(b); const sdk=b.sdk;
  ...
  check('C7 WL1: whiteLabelEnabled=true',              brand?.whiteLabelEnabled===true,           `wl=${brand?.whiteLabelEnabled}`);
  check('C7 WL2: hidePoweredBy=true',                  brand?.hidePoweredBy===true,               '');
  check('C7 WL3: poweredByText replaced',              brand?.poweredByText==='Powered by Acme',  `txt=${brand?.poweredByText}`);
  check('C7 WL4: companyName replaced',                brand?.companyName==='Acme Support',       `name=${brand?.companyName}`);
  check('C7 WL5: cornerRadius=pill',                   brand?.cornerRadius==='9999px',            `cr=${brand?.cornerRadius}`);
  check('C7 WL6: animationPreset=subtle',              brand?.animationPreset==='subtle',         `anim=${brand?.animationPreset}`);
  check('C7 WL7: whiteLabelEnabled in diagnostics',    d.whiteLabelEnabled===true,                '');
  check('C7 WL8: activeBrand is companyName',          d.activeBrand==='Acme Support',            `brand=${d.activeBrand}`);

  // Reset restores defaults
  sdk.resetBranding();
  const d2=sdk.getDiagnostics();
  check('C7 WL9: whiteLabelEnabled=false after reset', d2.whiteLabelEnabled===false,              '');
  check('C7 WL10: activeBrand reset to LeadFlow Chat', d2.activeBrand==='LeadFlow Chat',          `brand=${d2.activeBrand}`);
}
...
console.log('\n── C.7 Events ──────────────────────────────────────────────────');
{
  const b=makeBrowser({scriptDataset:{business:'biz_bev'}});
  await runBundle(b); const sdk=b.sdk;
  const evts={};
  sdk.eventBus.on('BRANDING_UPDATED',      p=>evts['branded']    =p);
  sdk.eventBus.on('THEME_UPDATED',         p=>evts['theme']      =p);
  sdk.eventBus.on('LOGO_UPDATED',          p=>evts['logo']       =p);
  sdk.eventBus.on('TYPOGRAPHY_UPDATED',    p=>evts['typo']       =p);
  sdk.eventBus.on('WHITE_LABEL_ENABLED',   p=>evts['wlOn']       =p);
  sdk.eventBus.on('WHITE_LABEL_DISABLED',  p=>evts['wlOff']      =p);
  ...
  sdk.applyBranding({
    ...
    whiteLabel:{hidePoweredBy:true,companyName:'Acme'},
  });

  check('C7 EV1: BRANDING_UPDATED emitted',            !!evts['branded'],                         '');
  check('C7 EV2: THEME_UPDATED emitted',               !!evts['theme'],                           '');
  check('C7 EV3: LOGO_UPDATED emitted',                !!evts['logo'],                            '');
  check('C7 EV4: TYPOGRAPHY_UPDATED emitted',          !!evts['typo'],                            '');
  check('C7 EV5: WHITE_LABEL_ENABLED emitted',         !!evts['wlOn'],                            '');
  check('C7 EV6: BRANDING_UPDATED has timestamp',      typeof evts['branded']?.timestamp==='string','');
  check('C7 EV7: BRANDING_UPDATED has companyName',    typeof evts['branded']?.companyName==='string','');
  check('C7 EV8: THEME_UPDATED has mode',              typeof evts['theme']?.mode==='string',     '');

  sdk.resetBranding();
  check('C7 EV9: BRANDING_UPDATED emitted on reset',   !!evts['branded'],                         '');
}
...
console.log('\n── C.7 Diagnostics ─────────────────────────────────────────────');
{
  const b=makeBrowser({scriptDataset:{business:'biz_bd7'}});
  await runBundle(b); const sdk=b.sdk;
  const d=sdk.getDiagnostics();

  check('C7 DG1: brandingEnabled in diagnostics',      typeof d.brandingEnabled==='boolean',      `enabled=${d.brandingEnabled}`);
  check('C7 DG2: whiteLabelEnabled in diagnostics',    typeof d.whiteLabelEnabled==='boolean',    '');
  check('C7 DG3: activeBrand in diagnostics',          typeof d.activeBrand==='string',           `brand=${d.activeBrand}`);
  check('C7 DG4: activePalette in diagnostics',        typeof d.activePalette==='string',         `pal=${d.activePalette}`);
  check('C7 DG5: activeTypography in diagnostics',     typeof d.activeTypography==='string',      '');
  check('C7 DG6: logoConfigured is boolean',           typeof d.logoConfigured==='boolean',       '');
  check('C7 DG7: avatarConfigured is boolean',         typeof d.avatarConfigured==='boolean',     '');
  check('C7 DG8: brandingEnabled=false initially',     d.brandingEnabled===false,                 `enabled=${d.brandingEnabled}`);
  check('C7 DG9: JSON-serialisable',
    (()=>{try{JSON.stringify(d);return true;}catch{return false;}})(), '');

  sdk.applyBranding({colors:{primary:'#abcdef'},whiteLabel:{companyName:'Test Co'}});
  const d2=sdk.getDiagnostics();
  check('C7 DG10: brandingEnabled=true after apply',   d2.brandingEnabled===true,                 `enabled=${d2.brandingEnabled}`);
  check('C7 DG11: activePalette=#abcdef after apply',  d2.activePalette==='#abcdef',              `pal=${d2.activePalette}`);
  check('C7 DG12: activeBrand=Test Co after apply',    d2.activeBrand==='Test Co',                `brand=${d2.activeBrand}`);
}

// ════════════════════════════════════════════════════════════
// C.7 — Lifecycle (destroy + re-init)
// ════════════════════════════════════════════════════════════
console.log('\n── C.7 Lifecycle ───────────────────────────────────────────────');
{
  const b=makeBrowser({scriptDataset:{business:'biz_lc7'}});
  await runBundle(b); const sdk=b.sdk;

  check('C7 LC1: branding exists after init',          sdk.branding!==null,                       '');
  sdk.applyBranding({colors:{primary:'#ff00ff'},whiteLabel:{companyName:'MyBrand'}});
  check('C7 LC2: branding applied',                    sdk.getDiagnostics().activeBrand==='MyBrand','');

  sdk.destroy();
  check('C7 LC3: branding=null after destroy',         sdk.branding===null,                       '');

  const s=await sdk.initialize({businessId:'biz_lc7b',position:'bottom-right',theme:'auto',primaryColor:'#6366f1'});
  check('C7 LC4: branding restored after re-init',     sdk.branding!==null&&s==='mounted',        `status=${s}`);
  check('C7 LC5: branding reset to defaults after re-init', sdk.getDiagnostics().brandingEnabled===false,'');
}
...
console.log('\n── Bundle ──────────────────────────────────────────────────────');
{
  ...
  check('BN: C.7 BRANDING_UPDATED', bundle.includes('BRANDING_UPDATED'),                    'C.7 event');
  check('BN: C.7 WHITE_LABEL',      bundle.includes('WHITE_LABEL_ENABLED'),                 'C.7 wl');
  check('BN: C.7 color engine',     bundle.includes('primaryHover'),                        'C.7 colors');
